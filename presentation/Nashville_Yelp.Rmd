...
load('../data/clean/nash_uber.RData')
...
df <- nash.hood %>% 
    left_join(
        nash.hood.uber %>% 
            select(id, distance, uberX, uberXL),
        by = c('neighborhood'='id')
    ) %>% 
    mutate(popup = paste0(
        neighborhood,
        '<BR>Rank: ', Rank, ' of ', n(),
...
datatable(
    data = df %>% 
        select(Rank, neighborhood, rating, review_count, count, distance, uberX, uberXL),
    colnames = c('Rank','Neighborhood', 'Rating', 'Reviews', 'Restaurants', 'Distance','UberX','UberXL'),
    class = 'row-border stripe compact order-column hover',
    options=list(pageLength = 10, dom = 'tip',
