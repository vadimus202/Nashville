...
## Uber Pricing Data {.smaller_font}
...
```{r uber_route,  fig.width=0.50*1125/72, fig.height=0.50*486/72}
...
The map on the next slide allows to zoom-in on individual Neighborhoods to find out where the best-rated restaurants are concentrated.  
...
__Yelp Ratings by Neighborhood__
```{r hood_map}
...
datatable(
    data = df %>% 
        ...
        select(Rank, neighborhood, rating, review_count, count, uberX, distance, duration),
    colnames = c('Rank','Neighborhood', 'Rating', 'Reviews', 'Restaurants','UberX', 'Distance', 'Minutes'),
    class = 'row-border stripe compact order-column hover',
    options=list(pageLength = 10, dom = 'tip',
...
## Nashville Hot Chicken {.smaller_font}
...
- The **color** of the circles represents the rank from 1 (Dark Red) to 20 (Pink).  
...
## Top 20 Music Venues {.smaller_table}
```{r top_music_venues}
...
## Top 20 Southern Restaurants  {.smaller_table} 
```{r}
  top_restr('southern')
```
...
```{r bbq_img, fig.width=0.70*460/72, fig.height=0.70*276/72}
...
## Top 20 Barbeque Restaurants  {.smaller_table} 
```{r}
  top_restr('bbq')
```
...
## Top 20 Steakhouses {.smaller_table}
```{r}
  top_restr('steak')
```
